		...
		<div class="container">
			...
			<div class="center">
				...
				if (isset($_POST['user']) && isset($_POST['pass']))
				{
					...
					$result = mysqli_query($conn,"SELECT user,hp,email FROM user WHERE (user='$user' OR hp='$hp' OR email='$email')");
					if ($result && mysqli_num_rows($result) > 0)
					{
						header ("location: daftar.php?pesan=gagaldaftar")
						?>
						<?php
					}
					else
					{
						$sql = "Insert into user (user,pass,hp,email,level) values ('$user','$pass','$hp','$email','level')";
						$result = mysqli_query($conn,$sql) or mysqli_error($conn);
						echo '<center><strong>Anda telah berhasil mendaftar</strong></center><br>'; 
					}					
				}
				?>
				<form action="login.php"><center class="mb-3"><input type="submit" class="btn btn-primary" name="login" value="Masuk"></center></form>	
			</div>
		</div>
	</div>
